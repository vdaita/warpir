...
__global__ void kernel(const __grid_constant__ kernel_globals g) {
  ...
  for (k_index = 0; (k_index < num_tiles); k_index = (k_index + 1)) {
    tma::load_async(As_0, g.A, {0, 0, row, k_index});
    tma::load_async(Bs_0, g.B, {0, 0, k_index, col});
    tma::load_async(As_1, g.A, {0, 0, row, (k_index + 1)});
    tma::load_async(Bs_1, g.B, {0, 0, (k_index + 1), col});
